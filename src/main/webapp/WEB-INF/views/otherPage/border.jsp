...
<div class="container">
			...
			<c:choose>
				...
				<c:when test="${!empty borderList }">
					<c:forEach var="list" items="${borderList }">
						<tr style = "cursor:pointer;" onclick="location.href='borderView${pagination.makeQuery(pagination.criteria.page)}&borderId=${list.borderId}'">					
	...
	<ul class="pagination">
		<c:if test="${pagination.prev }">
			<li><a class="page-link" href="border${pagination.makeQuery(pagination.startPage-1) }">Prev</a></li>
		</c:if> 
		<c:forEach begin="${pagination.startPage}" end="${pagination.endPage }" var="idx">
			<li <c:out value="${pagination.criteria.page == idx ? 'class=active' : '' }" />>
				<a href="border${pagination.makeQuery(idx) }">${idx }</a>
			</li>
		</c:forEach>
		<c:if test="${pagination.next && pagination.endPage > 0 }">
			<li><a class="page-link" href="border${pagination.makeQuery(pagination.endPage+1) }">Next</a></li> 
		</c:if>
	</ul>
